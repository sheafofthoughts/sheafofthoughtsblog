...
date: "2025-12-25"
description: >
  We introduce Gaussian integers and explain what it means, in this context, to be a sum of two squares. In particular, we will see what it means for a prime to be a sum of two squares in the Gaussian integers.
...
Let's recap what we have seen so far. We start by stating Fermat's Christmas theorem in terms of congruences. 
...
We now have a solution to @eq-gauss.
...
If $p+0i$ is not prime, then it can be written as a product of two factors:
$$
	p+0i=(a+bi)(c+di)
$$ 
...
Since $p\equiv_4 1$, it means that $p-1$ is divisible by $4$, which means that $p-1=4k$ for some integer $k$. We have then
$$
...
<details class="solution">
  ...
  By definition of $k$, this forces $q\mid k$.

  But then $q$ divides both $k$ and $k^2+1$, hence it divides their difference $(k^2+1)-k^2=1$, impossible.
  Contradiction.
